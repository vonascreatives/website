...
/**
 * @deprecated This file contains mock blog data for fallback purposes only.
 * All blog components should now use real CMS data from Sanity via getNewsData().
 * This file is kept for backward compatibility and fallback scenarios.
 * 
 * Components using CMS data:
 * - BlogListArea (uses getNewsData)
 * - BlogModernArea (uses getNewsData) 
 * - BlogClassicArea (uses getNewsData)
 * - BlogDetailsRelatedPosts (uses getNewsData with fallback)
 * - BlogClassicSlider (uses featuredArticles prop with fallback)
 * - BlogSidebar (uses recentPosts prop with fallback)
 */
