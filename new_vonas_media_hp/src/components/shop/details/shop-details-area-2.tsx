...
export default function ShopDetailsAreaTwo({ product }: IProps) {
  // Use creator image if available, otherwise fallback to default images
  const creatorImage = product?.image;
  const hasCreatorImage = creatorImage && typeof creatorImage === 'string' && creatorImage.includes('http');
    ...
    <div className="tp-shop-details-2-area tp-product-details-2-style pt-170">
      <div className="container container-1300">
        <div className="row align-items-start">
                  ...
                  >
                    <div className="tp-product-details-nav-main-thumb">
                      <Image
                        className="w-100"
                        src={creatorImage}
                        alt={product?.imageAlt || product?.name || "Creator image"}
                        width={600}
                        height={600}
                  ...
                  {hasCreatorImage ? (
                    ...
                    >
                      <Image
                        className="w-100"
                        src={creatorImage}
                        alt={product?.imageAlt || "Creator thumbnail"}
                        width={80}
                        height={74}
          ...
          <div className="col-xl-6 col-lg-6">
            <DetailsRightWrap product={product} />
          </div>
        </div>
      </div>
    </div>
