...
export default function FileSys({
  ...
  return (
    <div className="dark:bg-stone-950/25 bg-stone-50/25 rounded-md p-4 px-3 border flex flex-col gap-1.5 font-mono max-w-full overflow-x-auto">
      {items.map((f, index) => {
        if (isFile(f)) return <File {...f} key={`${f.name}-${index}`} />;
        return <Folder {...f} key={`${f.name}-${index}`} sorted={sorted} />;
      })}
    </div>
  );
}
