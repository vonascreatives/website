// Run with: sanity exec scripts/fix-images-cli.js --with-user-token
...
async function fixBlogPosts() {
  ...
  const posts = await client.fetch(`*[_type == "post" && (coverImage == null || length(coverImage) == 0)]`)
  
  for (const post of posts) {
    console.log(`Fixing post: ${post.title}`)
    
    await client.patch(post._id).set({
      coverImage: [createImageWithAlt(`Cover image for ${post.title}`)]
    }).commit()
