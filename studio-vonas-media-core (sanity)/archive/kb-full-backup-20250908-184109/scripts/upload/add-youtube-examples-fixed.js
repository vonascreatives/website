...
// Sample YouTube channels with proper Sanity structure
...
async function addYouTubeExamplesFixed() {
  try {
    console.log('🔄 Fixing YouTube channel examples with proper Sanity structure...\\n')
    
    // First, delete the existing corrupted documents
    console.log('🗑️ Removing corrupted documents...')
    for (let i = 1; i <= 5; i++) {
      try {
    ...
    console.log('\\n✨ Creating properly structured documents...')
    ...
    console.log('🎉 Successfully fixed and added YouTube channel examples!')
    console.log(`📊 Added ${sampleYouTubeChannels.length} properly structured YouTube channels:`)
    ...
    console.log('   • Fixed block content structure with markDefs and style')
    console.log('   • Added proper _type properties to objects')
    console.log('\\n🌐 Check them out at:')
    console.log('   • Local: http://localhost:3333/')
    console.log('   • Deployed: https://vonas-media.sanity.studio/')
    
  } catch (error) {
    console.error('❌ Error fixing YouTube examples:', error)
  }
}

// Run if executed directly
if (require.main === module) {
  addYouTubeExamplesFixed()
}

module.exports = { addYouTubeExamplesFixed }
